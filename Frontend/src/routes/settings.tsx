...
function SettingsPage() {
  return (
    <AppLayout>
      <PageHeader
        title="System Configurations & RBAC"
        subtitle="Global depot preferences and role-based access matrix."
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,45fr)_minmax(0,55fr)] gap-6">
        <Card className="p-6">
          ...
          <div className="space-y-4">
            ...
            <Button className="w-full h-11 mt-2">Save Configuration Changes</Button>
          </div>
        </Card>

        <Card className="p-0 overflow-hidden">
          <div className="p-5 border-b">
            <h3 className="font-semibold">RBAC Permission Matrix</h3>
            <p className="text-xs text-muted-foreground mt-0.5">
              Module access by operational role
            </p>
          </div>
          <Table>
            <TableHeader>
                ...
                <TableRow key={r.role}>
                  ...
                  {r.cells.map((c, i) => (
                    <TableCell key={i} className="text-center">
                      <div className="flex justify-center">{renderCell(c)}</div>
                    </TableCell>
                  ))}
                </TableRow>
              ))}
          ...
          <div className="p-4 border-t bg-muted/30 text-[11px] text-muted-foreground flex items-center gap-4 flex-wrap">
            <span className="flex items-center gap-1.5">
              <Check className="h-3.5 w-3.5 text-status-available" /> Full access
            </span>
            <span className="flex items-center gap-1.5">
              <Eye className="h-3.5 w-3.5 text-status-active" /> Read-only
            </span>
            <span className="flex items-center gap-1.5">
              <Minus className="h-3.5 w-3.5 text-muted-foreground/60" /> No access
            </span>
          </div>
        </Card>
      </div>
